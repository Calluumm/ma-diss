...
def main() -> None:
    ...
    # Keep one x-position per interval (based on active channel rows).
    ...
    change_vals = [r.change_ha for r in rows]
    ...
    draw.text((60, 102), "Full and zoomed views are shown so large 2019-2020 swings are retained without flattening later intervals.", fill=(90, 86, 77), font=f_b)

    # Panel A: rainfall bars
    ...
    draw.text((a_left + 740, a_top + 8), "Rainfall (mm)", fill=(60, 90, 120), font=f_s)

    # Panel B: total mapped change on log10 scale
    ...
    for mask in MASKS:
        ...
        for r in series:
            year = r.post_date[:4]
            x = xpos_b[year]
            clamped_ha = max(10.0, r.change_ha)
            yv = math.log10(clamped_ha + 1.0)
    ...
    tick_values = [10, 25, 50, 100, 250, 500, 1000, 2500]
    b_ticks = []
    for tv in tick_values:
        lv = math.log10(tv + 1.0)
        if lv <= lg_max:
            b_ticks.append((lv, str(tv)))
    ...
    draw.text((b_left + 16, b_top + 8), "Y-axis starts at 10 ha; spacing is log10 (triangles mark values below 10 ha).", fill=(92, 88, 78), font=f_s)

    draw.text((b_left, b_bottom + 30), "Post-scene year", fill=(70, 68, 62), font=f_s)

    # Panel C: directional balance index (restored)
    c_left, c_top, c_right, c_bottom = 140, 870, 1210, 1450
    ...
    draw.text((c_left + 16, c_top + 8), "(gain-loss)/(gain+loss): +1 gain-dominant, -1 loss-dominant.", fill=(92, 88, 78), font=f_s)

    draw.text((c_left, c_bottom + 30), "Post-scene year", fill=(70, 68, 62), font=f_s)

    # Panel D: 2023/2025 focus with long-context envelope
    ...
    # Log-scaled horizontal mapping spreads low-change years so 2023/2025 are readable.
    ...
    # Shared scale for both focus rows.
    ...
    def draw_focus_row(row_y: int, year_label: str, value_m2: float | None, col: tuple[int, int, int]) -> None:
        # Baseline context: min-max whisker and IQR box.
        ...
        # Compact value callout and percentile rank.
    ...
    draw.text((track_left, d_bottom - 52), "Lower change", fill=(92, 88, 78), font=f_s)
    t_hi = "Higher change"
    t_hi_box = draw.textbbox((0, 0), t_hi, font=f_s)
    draw.text((track_right - (t_hi_box[2] - t_hi_box[0]), d_bottom - 52), t_hi, fill=(92, 88, 78), font=f_s)

    # Compact context legend avoids dense labels on the skewed scale.
    ctx_x, ctx_y = d_right - 355, d_top + 26
    draw.rounded_rectangle((ctx_x, ctx_y, ctx_x + 325, ctx_y + 96), radius=8, fill=(248, 244, 236), outline=(213, 206, 192), width=1)
    ...
    draw.text((d_left + 16, d_top + 8), "Rows show only 2023 and 2025; background bars summarise all years (min-max and IQR, log-spaced).", fill=(92, 88, 78), font=f_s)
